                                    ...
                                    <div class="row">
                                        <div class="col">
                                            <select name="user" class="form-control">
                                                ...
                                                <?php 
                                                $query = mysqli_query($conn,"SELECT * FROM users WHERE usertype != 0");
                                                while($results = mysqli_fetch_assoc($query)){ 
                                                    $usrid = $results['id'];
                                                    if($results['usertype'] == 1){
                                                        $get = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM teachers WHERE user_id = $usrid"));
                                                        $name = $get['fullname'].'(مدرس)';
                                                    }elseif($results['usertype'] == 2){
                                                        $get = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM students WHERE user_id = $usrid"));
                                                        $name = $get['fullname'].'(طالب)';
                                                    }
                                                    ?>
