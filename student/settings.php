...
$sqlq = "SELECT * FROM students WHERE user_id = $user_id";
...
if(isset($_POST['update'])) {
    ...
    if($_FILES['logo']['size'] != 0 && $_FILES['logo']['error'] == 0){
        ...
        if( in_array($ext1, $x) ) {
            ...
            move_uploaded_file($new_logo['tmp_name'], '../images/students/'.$newimgName);
            $newsqll = "UPDATE students SET image='$newimgName', phone='$new_phone', email='$new_email', address='$new_address' WHERE user_id=$user_id";
            if(mysqli_query($conn, $newsqll)) {
                $alert = '<div class="alert alert-soft-success d-flex" role="alert">
                <div class="text-body">تم تحديث بيانات الطالب بنجاح</div>
                </div>';
            }else {
                $alert = '<div class="alert alert-soft-danger d-flex" role="alert">
                <div class="text-body">لم يتم تحديث بيانات الطالب بنجاح</div>
                </div>';
            }
        } else {
            $alert = '<div class="alert alert-soft-danger d-flex" role="alert">
        ...
        $newsqll = "UPDATE students SET phone='$new_phone', email='$new_email', address='$new_address' WHERE user_id='$user_id'";
        if(mysqli_query($conn, $newsqll)) {
            $alert = '<div class="alert alert-soft-success d-flex" role="alert">
                <div class="text-body">تم تحديث بيانات الطالب بنجاح</div>
                </div>';
...
<div class="mdk-drawer-layout__content page">
                    ...
                    <div class="container-fluid page__container">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div>
                                    <img width="100%" src="../images/students/<?= $image ?>" alt="">
                                </div>
                            </div>
                        </div>
                    </div>
